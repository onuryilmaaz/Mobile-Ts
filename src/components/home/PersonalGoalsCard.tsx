...
function getProgress(logs: TrackerLog[], activity: GoalActivity): number {
  ...
  switch (activity) {
    case 'quran':
      return filtered.reduce((s, l) => s + (Number(l.value.pages) || 0), 0);
    case 'dhikr':
      return filtered.reduce((s, l) => s + (Number(l.value.count) || 0), 0);
    case 'nafile':
      return filtered.reduce((s, l) => s + (Number(l.value.rakaat) || 0), 0);
    case 'fasting':
      return filtered.length;
    case 'dua':
      return filtered.reduce((s, l) => s + (Number(l.value.minutes) || 0), 0);
    case 'memorization':
      return filtered.reduce(
        (s, l) => s + (Number(l.value.new_ayets) || 0) + (Number(l.value.revision_ayets) || 0),
        0
      );
    default:
      return 0;
...
function buildCompleteValue(activity: GoalActivity, remaining: number): Record<string, any> {
  switch (activity) {
    ...
    case 'dhikr':
      return { subtype: 'Genel', count: remaining };
    case 'nafile':
      return { type: 'diger', rakaat: remaining };
    case 'fasting':
      return { type: 'nafile' };
    case 'dua':
      return { type: 'Genel', minutes: remaining };
    case 'memorization':
      return { new_ayets: remaining, revision_ayets: 0 };
  }
}
          ...
          <View className="flex-row items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            <View>
              ...
              <Text className="text-xs text-slate-400 dark:text-slate-500">
                Aktif hedefler ana ekranda ve widget&rsquo;ta görünür
              </Text>
            </View>
            <TouchableOpacity
              onPress={onClose}
            ...
            {goals.map((g) => (
              <View
                ...
                <TouchableOpacity
                  ...
                  {/* Toggle */}
                  <View
                    className={`h-6 w-11 rounded-full ${g.enabled ? 'bg-teal-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                      ...
                      {[
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            const next = Math.max(
                              1,
                              (drafts[g.activity] ? parseInt(drafts[g.activity]) : g.target) + delta
                            );
                      ...
                      {[
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                            const next =
                              (drafts[g.activity] ? parseInt(drafts[g.activity]) : g.target) +
                              delta;
                            setDrafts((p) => ({ ...p, [g.activity]: String(next) }));
                            onTargetChange(g.activity, next);
                          }}
...
export function PersonalGoalsCard() {
  ...
  const handleComplete = useCallback(
    async (activity: GoalActivity, target: number, progress: number) => {
      if (!isAuthenticated) {
  ...
  const handleToggle = useCallback(
    (activity: GoalActivity, enabled: boolean) => {
  ...
  const handleTarget = useCallback(
    (activity: GoalActivity, target: number) => {
      ...
      <>
        <TouchableOpacity
          ...
          <View className="flex-1">
            ...
            <Text className="text-xs text-slate-400">Widget&rsquo;ta da görünecek</Text>
          </View>
  ...
  return (
    <>
      <View className="mx-4 mb-4 overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <TouchableOpacity
          ...
          {allDone ? (
            <View className="rounded-full bg-teal-50 px-2.5 py-0.5 dark:bg-teal-500/15">
              <Text className="text-[10px] font-black text-teal-600 dark:text-teal-400">
                TÜMÜ TAMAM 🎉
              </Text>
            </View>
          ) : (
        ...
        <View className="gap-3.5 px-4 py-3">
          {goalItems.map((g) => {
                    ...
                    {!done && (
                      <TouchableOpacity
                        ...
                        <Text
                          ...
                          style={{ color: isCompleting ? '#94a3b8' : g.colorHex }}>
