...
function StatCard({ icon, label, value, color }: { icon: any; label: string; value: string | number; color: string }) {
  return (
    <View className="flex-1 mx-1.5 rounded-[20px] border border-slate-100 bg-white p-4 shadow-sm items-center">
      <View className="mb-2 h-10 w-10 items-center justify-center rounded-2xl" style={{ backgroundColor: `${color}18` }}>
        <Ionicons name={icon} size={20} color={color} />
      </View>
      <Text className="text-xl font-black text-slate-800">{value}</Text>
      <Text className="mt-0.5 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400">{label}</Text>
    </View>
  );
}
    ...
    <Screen className="bg-slate-50" safeAreaEdges={['left', 'right']}>
      <ScrollView
        ...
        <View className="px-4 mb-6">
          <Text className="mb-3 text-lg font-black text-slate-800">30 Günlük Özet</Text>
          <View className="flex-row">
            <StatCard icon="sunny" label="Toplam Namaz" value={totalMonthPrayers} color="#0f766e" />
            <StatCard icon="flame" label="Aktif Gün" value={activeDays} color="#f59e0b" />
            <StatCard icon="checkmark-circle" label="Tamamlama" value={`%${completionRate}`} color="#6366f1" />
          </View>
        </View>

        {/* Weekly Bar Chart */}
        {weeklyData && (
          <View className="mx-4 mb-6 overflow-hidden rounded-[24px] border border-slate-100 bg-white p-4 shadow-sm">
            <Text className="mb-4 text-base font-black text-slate-800">Haftalık Namaz Grafiği</Text>
            <BarChart
              data={{
            ...
            <View className="mt-2 flex-row items-center gap-2">
              ...
              <Text className="text-xs text-slate-500">Kılınan namaz sayısı (max 5)</Text>
            </View>
          </View>
        )}
        ...
        <View className="mx-4 mb-6 overflow-hidden rounded-[24px] border border-slate-100 bg-white p-4 shadow-sm">
          <Text className="mb-4 text-base font-black text-slate-800">Vakit Bazlı Analiz</Text>
          <Text className="mb-4 text-xs text-slate-400">Son 7 gün — Hangi vakitte daha başarılısın?</Text>

          {breakdown.length === 0 ? (
            <View className="items-center py-8">
              <Ionicons name="bar-chart-outline" size={36} color="#cbd5e1" />
              <Text className="mt-2 text-sm text-slate-400">Henüz yeterli veri yok</Text>
            </View>
          ) : (
              ...
              <View key={item.prayer} className="mb-4">
                <View className="mb-1.5 flex-row items-center justify-between">
                  <View className="flex-row items-center gap-2">
                    ...
                    <Text className="text-sm font-bold text-slate-700">{PRAYER_LABELS[item.prayer] || item.prayer}</Text>
                  </View>
                  <View className="flex-row items-center gap-2">
                    {item.kaza > 0 && (
                      <View className="rounded-full bg-orange-100 px-2 py-0.5">
                        <Text className="text-[10px] font-bold text-orange-600">{item.kaza} Kaza</Text>
                      </View>
                    )}
                    <Text className="text-sm font-black text-slate-800">{item.total}/7</Text>
                  </View>
                </View>
        ...
        {/* Streak & Points */}
        <View className="mx-4 mb-6 overflow-hidden rounded-[24px] border border-slate-100 bg-white p-4 shadow-sm">
          <Text className="mb-4 text-base font-black text-slate-800">Genel Bilgiler</Text>
          <View className="flex-row flex-wrap gap-3">
            {[
              ...
              <View key={item.label} className="w-[47%] rounded-[16px] border border-slate-100 bg-slate-50 p-3">
                <View className="flex-row items-center gap-2 mb-1">
                  <Ionicons name={item.icon as any} size={14} color={item.color} />
                  <Text className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{item.label}</Text>
                </View>
                <Text className="text-base font-black text-slate-800">{item.value}</Text>
              </View>
            ))}
          </View>
        </View>
