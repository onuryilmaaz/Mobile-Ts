...
function PodiumItem({
  ...
  return (
    <View style={{ width: width * 0.28 }} className="items-center">
      ...
      <Text
        className="mb-0.5 text-center text-[11px] font-bold text-slate-900 dark:text-white"
        numberOfLines={1}>
        {user.first_name}
      </Text>
      <View className="rounded-full bg-teal-50 px-2 py-0.5 dark:bg-teal-500/15">
        <Text className="text-[10px] font-black text-teal-700 dark:text-teal-400">
    ...
    <Screen safeAreaEdges={['left', 'right']}>
      <ScrollView
          ...
          <View
            ...
            <View className="mb-3 flex-row items-center justify-between">
              <View>
                ...
                <Text className="text-xl font-black" style={{ color: level.color }}>
                  {level.name}
                </Text>
              </View>
              <View
            ...
            <View className="mt-2 flex-row justify-between">
              <Text className="text-[10px] font-bold text-slate-500 dark:text-slate-300">
                %{level.progressPercent} tamamlandı
              </Text>
              {nextLevel && (
                <Text className="text-[10px] font-bold text-slate-500 dark:text-slate-300">
            ...
            <View className="mt-4 flex-row gap-1">
              {LEVELS.map((l) => (
                <View
                  ...
                  style={{
                    backgroundColor:
                      level.level >= l.level ? l.color : isDark ? '#1e293b' : '#e2e8f0',
                  }}
                />
              ))}
            </View>
        ...
        <View className="px-6 pt-2">
          {badgesByCategory.map((cat) => (
            <View key={cat.category} className="mb-8">
              <View className="mb-4 flex-row items-center gap-2">
                ...
                <Text className="text-base font-black capitalize text-slate-900 dark:text-white">
                  {cat.category}
                </Text>
                <Text className="text-xs font-bold text-slate-500 dark:text-slate-400">
                  ({cat.badges.filter((b: any) => badges?.some((e) => e.badge_id === b.id)).length}/
                  {cat.badges.length})
                </Text>
              </View>
                    ...
                    <TouchableOpacity
                      ...
                      <View
                        ...
                        <Ionicons
                          ...
                          size={32}
                          color={
                            isEarned ? badgeDesc.color || '#059669' : isDark ? '#4b5563' : '#94a3b8'
                          }
                        />
                      </View>
                      <Text
                        className="px-1 text-center text-[10px] font-bold text-slate-500 dark:text-slate-400"
                        numberOfLines={2}>
                        {badgeDesc.name}
                      </Text>
                    </TouchableOpacity>
          ...
          <View className="mb-6">
            <Text className="mb-8 text-2xl font-black text-slate-900 dark:text-white">
              Sıralama
            </Text>
            ...
            <View
                ...
                return (
                  <View
                    ...
                    className={`mb-2 flex-row items-center rounded-3xl p-4 ${
                      isMe
                        ? 'bg-teal-600 shadow-sm shadow-teal-600/30 dark:bg-teal-500'
                        : 'bg-transparent'
                    }`}>
                    <Text
                      className={`w-8 text-xs font-black ${isMe ? 'text-white' : 'text-slate-500 dark:text-slate-300'}`}>
                      {idx + 4}
                    </Text>
                    <View
                      ...
                      {item.avatar_url ? (
                        ...
                        <Ionicons
                          name="person"
                          size={20}
                          color={isMe ? '#fff' : isDark ? '#4b5563' : '#94a3b8'}
                        />
                      )}
                    </View>
                    <View className="flex-1">
                      <Text
                        className={`text-sm font-bold ${isMe ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                        {item.first_name} {item.last_name || ''}
                      </Text>
                      <Text
                        className={`text-[9px] font-medium ${isMe ? 'text-white/80' : 'text-slate-500 dark:text-slate-300'}`}>
                        {item.current_streak} Günlük Seri
                      </Text>
                    </View>
                    <View
                      className={`items-end rounded-full px-3 py-1 ${
                        isMe ? 'bg-white/20' : 'bg-teal-50 dark:bg-teal-500/15'
                      }`}>
                      <Text
                        className={`text-xs font-black ${isMe ? 'text-white' : 'text-teal-700 dark:text-teal-400'}`}>
                        {item.total_points}
                      </Text>
                    </View>
                  </View>
              ...
              {!leaderboard?.length && !isLoading && (
                <View className="items-center p-12">
                  ...
                  <Text className="text-center font-bold text-slate-500 dark:text-slate-300">
