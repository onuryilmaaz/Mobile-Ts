...
type Props = {
  navigation: NativeStackNavigationProp<GroupStackParamList, 'GoalCreate'>;
  route: RouteProp<GroupStackParamList, 'GoalCreate'>;
};
...
export default function GoalCreateScreen({ navigation, route }: Props) {
  ...
  async function handleCreate() {
    ...
    try {
      setLoading(true);
      await groupApi.createGoal(groupId, {
        title: title.trim(),
        goal_type: goalType,
        target_value: target,
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <Text className="mb-8 text-sm text-slate-500 dark:text-slate-400">
            Grubunuz için bir ibadet hedefi belirleyin
          </Text>
          ...
          <Input
            ...
            placeholder="2024-12-31 (boş = süresiz)"
            keyboardType="numbers-and-punctuation"
            maxLength={10}
          />
