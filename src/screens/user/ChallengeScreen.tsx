...
const TYPE_COLORS: Record<string, { bg: string, text: string, raw: string }> = {
  weekly:  { bg: 'bg-teal-700/10 dark:bg-teal-500/15', text: 'text-teal-700 dark:text-teal-400', raw: '#0f766e' },
  monthly: { bg: 'bg-indigo-600/10 dark:bg-indigo-500/15', text: 'text-indigo-600 dark:text-indigo-400', raw: '#4f46e5' },
  special: { bg: 'bg-amber-500/10 dark:bg-amber-500/15', text: 'text-amber-600 dark:text-amber-400', raw: '#f59e0b' },
};
    ...
    <Animated.View
      ...
      <View className={`flex-row items-center justify-between px-5 py-3 ${typeStyles.bg}`}>
        <View className="flex-row items-center gap-2">
          <View className={`h-7 w-7 items-center justify-center rounded-xl bg-black/5 dark:bg-white/10`}>
      ...
      <View className="px-5 py-4">
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center gap-1.5">
            ...
            <Text className="text-sm font-black text-slate-900 dark:text-white">+{challenge.bonus_points} Puan</Text>
          </View>
      ...
      <ScrollView
        ...
        <View className="mx-4 mb-6 overflow-hidden rounded-[28px] bg-teal-700 p-5 dark:bg-teal-900">
          ...
          <View className="flex-row gap-6">
            <View>
              ...
              <Text className="text-[10px] font-bold uppercase tracking-widest text-white/80">Aktif</Text>
            </View>
            <View className="w-[1px] bg-white/10" />
            <View>
              <Text className="text-2xl font-black text-white">{completedChallenges.length}</Text>
              <Text className="text-[10px] font-bold uppercase tracking-widest text-white/80">Tamamlanan</Text>
            </View>
            <View className="w-[1px] bg-white/10" />
            <View>
              <Text className="text-2xl font-black text-white">{availableChallenges.length}</Text>
              <Text className="text-[10px] font-bold uppercase tracking-widest text-white/80">Bekleyen</Text>
            </View>
          </View>
        </View>
        ...
        {joinedChallenges.length > 0 && (
          <View className="mb-4 px-4">
            <Text className="mb-3 text-base font-black text-slate-900 dark:text-white">Devam Eden</Text>
        ...
        {availableChallenges.length > 0 && (
          <View className="mb-4 px-4">
            <Text className="mb-3 text-base font-black text-slate-900 dark:text-white">Katılabileceklerin</Text>
        ...
        {completedChallenges.length > 0 && (
          <View className="mb-4 px-4">
            <Text className="mb-3 text-base font-black text-slate-900 dark:text-white">Tamamlananlar ✅</Text>
