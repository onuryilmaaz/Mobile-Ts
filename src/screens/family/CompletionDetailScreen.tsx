      ...
      <ScrollView
        ...
        <View className="items-center gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-black/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
          ...
          <View className="items-center gap-1">
            <Text className="text-xl font-black text-slate-900 dark:text-white">{item.child_name}</Text>
            <Text className="text-sm text-slate-500 dark:text-slate-400">{item.title} görevi tamamlandı</Text>
          </View>
        ...
        {item.evidence_url && (
          <View className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
            <Image source={{ uri: item.evidence_url }} style={{ width: '100%', height: 200 }} resizeMode="cover" />
          </View>
        )}
        ...
        <View className="flex-row gap-3">
          <TouchableOpacity
            ...
            <Ionicons name="close-circle-outline" size={18} color={isDark ? '#f87171' : '#ef4444'} />
